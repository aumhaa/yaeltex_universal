...
class MonoChannelStripComponent(ChannelStripComponentBase):
	...
	def __init__(self, *a, **k):
		...
		for index in range(8):
			self._device_provider[index] = ChannelStripStaticDeviceProvider()
			self._device_component[index] = ChannelStripDeviceComponent(device_provider = self._device_provider[index], device_bank_registry = DeviceBankRegistry(), *a, **k)
			self._device_component[index]._show_msg_callback = lambda message: None
	...
	def set_summed_output_meter_level_control(self, control):
		...
		if control != self._summed_output_meter_level_control:
			...
			# if control != None:
			# 	debug('restarting _vu_sum_task')
				#self._vu_sum_task.restart()
			self.update()
	...
	def set_parameter_controls(self, controls):
		self._device_component[0].set_parameter_controls(controls)


	def set_parameter_controls2(self, controls):
		self._device_component[1].set_parameter_controls(controls)


	def set_parameter_controls3(self, controls):
		self._device_component[2].set_parameter_controls(controls)


	def set_parameter_controls4(self, controls):
		self._device_component[3].set_parameter_controls(controls)


	def set_parameter_controls5(self, controls):
		self._device_component[4].set_parameter_controls(controls)


	def set_parameter_controls6(self, controls):
		self._device_component[5].set_parameter_controls(controls)


	def set_parameter_controls7(self, controls):
		self._device_component[6].set_parameter_controls(controls)


	def set_parameter_controls8(self, controls):
		self._device_component[7].set_parameter_controls(controls)


	def set_parameter_controls_on_off_buttons(self, controls):
		for device, button in zip_longest(self._device_component, controls or [], fillvalue = None):
	...
	def _update_device_selection(self):
		...
		for index in range(8):
			...
			if track and device_to_select == None and len(track.devices) > index:
				device_to_select = track.devices[index]
			self._device_provider[index].device = device_to_select
	...
	def disconnect(self):
		for index in range(8):
			self._device_component[index]._get_device = lambda: None
...
class MonoMixerComponent(MixerComponentBase):
	...
	def set_return_controls(self, controls):
		...
		for channel_strip, control in zip_longest(self._return_strips, controls or []):
			if liveobj_valid(channel_strip):
